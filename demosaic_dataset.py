...
def mosaic_mask_video(video, noise_level=5/255., pattern="RGGB"):
	...
	# create stacked and flattened versions of the Bayer array
	bayer_flat = degraded_video.sum(dim=2)

	bayer_stack = torch.zeros((B, N, H // 2, W // 2, 4), device=video.device)
	bayer_stack[..., 0] = bayer_flat[..., 0::2, 0::2]
	bayer_stack[..., 1] = bayer_flat[..., 0::2, 1::2]
	bayer_stack[..., 2] = bayer_flat[..., 1::2, 0::2]
	bayer_stack[..., 3] = bayer_flat[..., 1::2, 1::2]

	return degraded_video, mask, bayer_flat, bayer_stack


def demosaic_video_dataset(model, dataloader, init_type="mosaic", noise_level=0./255, admm_alpha=0.25, admm_denoiser_level=20./255, admm_iters=200, save_frames=False, save_graphs=False, output_folder=None, device=torch.device("cpu"), verbose=1):
	...
	with torch.no_grad():
		for b, batch in enumerate(dataloader):
				...
				if save_frames:
					...
					tensor_to_images(restored_video.squeeze(), f'{vid_name}_restored', os.path.join(image_folder, f"{vid_name}_mosaicked_RGGB_sigma{noise_level*255}_{denoiser_name}_alpha{admm_alpha}_s{int(admm_denoiser_level*255)}_{admm_iters}iters_restored"))
				if save_graphs:
					...
					vid_name = batch['video_name'][0]
					denoiser_name = model.__class__.__name__
					graph_subfolder = f"{os.path.join(graph_folder, f'{vid_name}_mosaicked_RGGB_sigma{noise_level*255}_{denoiser_name}_alpha{admm_alpha}_s{int(admm_denoiser_level*255)}_{admm_iters}iters')}"
	...
	for i, current_model in enumerate(model_list):
		...
		for s in args['denoiser_levels']:
			...
			for sigma in args['sigmas']:
				...
				for alpha in args['alphas']:
					print(f'alpha={alpha}')
					vid_names, psnrs_noisy, ssims_noisy, psnrs_out, ssims_out, runtimes, psnrs_x_iters, psnrs_z_iters, x_grads_iters, z_grads_iters, x_minus_zs_iters, drs_iters, avg_psnr_noisy, avg_ssim_noisy, avg_psnr_out, avg_ssim_out, avg_runtime = demosaic_video_dataset(model, dataloader, init_type=args['init'], noise_level=sigma/255., admm_alpha=alpha, admm_denoiser_level=s/255., admm_iters=args['max_iters'], save_frames=args['save_frames'], save_graphs=args['save_graphs'], output_folder=args['logdir'], device=device, verbose=args['verbose'])
...
if __name__ == "__main__":
	...
	print("\n### Running video PnP-ADMM demosaicking ###")
	print("> Parameters:")
