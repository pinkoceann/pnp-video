...
from zhang_utils import utils_sisr, utils_pnp, utils_image
from sr_dataset import lr_video
...
def sr_video_dataset(model, dataloader, kernel="gaussian25-1.6", scale_factor=2, x8=False, noise_level=1./255, max_denoiser_level=49./255, max_iters=24, save_frames=False, output_folder=None, device=torch.device("cpu"), verbose=1):
	...
	with torch.no_grad():
		for b, batch in enumerate(dataloader):
				...
				for n, img in enumerate(degraded_video[0]):  # for each image
					...
					FB, FBC, F2B, FBFy = utils_sisr.pre_calculate(img_L_tensor, k_tensor, scale_factor)
					for i in range(max_iters):
						# --------------------------------
						# # step 1: data fidelity term
						# --------------------------------
						tau = rhos[i].float().repeat(1, 1, 1, 1)
						x = utils_sisr.data_solution(x.float(), FB, FBC, F2B, FBFy, tau, scale_factor)
						# --------------------------------
						# step 2, denoiser
						# --------------------------------
						if x8:
				...
				if verbose >= 2:
					print(f"video: {batch['video_name'][0]:<18} PSNR/SSIM bicubic: {psnr_noisy:<2.2f}/{ssim_noisy:.4f}, PSNR/SSIM out: {psnr_out:<2.2f}/{ssim_out:.4f} \t runtime: {runtime:.3f}s/frame")
				...
				if save_frames:
					...
					tensor_to_images(restored_video.squeeze(), f'{vid_name}_restored', os.path.join(image_folder, f"{vid_name}_sr_sf{scale_factor}_kernel_{kernel}_sigma{noise_level*255}_{denoiser_name}_s{int(max_denoiser_level*255)}_DPIR_{max_iters}iters_restored"))
	...
	if verbose >= 1:
		print(f'DPIR model: {model.__class__.__name__:<18} PSNR/SSIM bicubic: {avg_psnr_noisy:<2.2f}/{avg_ssim_noisy:.4f}, PSNR/SSIM out: {avg_psnr_out:<2.2f}/{avg_ssim_out:.4f} \t runtime: {avg_runtime:.3f}s/frame\n')
			...
			for sf in args['scale_factors']:
				...
				for sigma in args['sigmas']:
					...
					if no_kernel:
						...
						no_kernel = False
					res_dict[model.__class__.__name__][f"s={s}"][f"sf={sf}"][f"sigma={sigma}"] = {'avg_psnr_noisy': float(avg_psnr_noisy), 'avg_ssim_noisy': float(avg_ssim_noisy), 'avg_psnr_out': float(avg_psnr_out), 'avg_ssim_out': float(avg_ssim_out), 'avg_runtime': float(avg_runtime)}
					for v, vid_name in enumerate(vid_names):
						res_dict[model.__class__.__name__][f"s={s}"][f"sf={sf}"][f"sigma={sigma}"][vid_name] = {'psnr_noisy': float(psnrs_noisy[v]), 'ssim_noisy': float(ssims_noisy[v]), 'psnr_out': float(psnrs_out[v]), 'ssim_out' : float(ssims_out[v]), 'runtime' : runtimes[v]}
