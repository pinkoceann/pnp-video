...
'''
from https://github.com/cszn/KAIR/blob/master/models/network_unet.py
# ====================
# Residual U-Net
# ====================
citation:
...
class DRUNet(nn.Module):
    def __init__(self, in_nc=4, out_nc=3, nc=[64, 128, 256, 512], nb=4, act_mode='R', downsample_mode='strideconv', upsample_mode='convtranspose', bias=False):
...
if __name__ == '__main__':
    x = torch.rand(1,4,256,256)  # 4 channels suggest 3 RGB channels concatenated with a per-pixel noise map
    net = DRUNet()
    net.eval()
    with torch.no_grad():
        y = net(x)
